...
void					handle_var_without_value(t_gc *gc, t_env *env,
							char *arg);
...
void					handle_exec_error(t_gc *gc, t_env *env, char *cmd,
							char *err_msg);
...
int						handle_output_redirection(t_cmd *cmd, char *filename,
							int flags);
int						handle_heredoc_redirection(t_gc *gc, t_cmd *cmd,
							t_token *filename_token);
int						handle_redirection(t_gc *gc, t_cmd *cmd,
							t_token **tokens);
int						check_redirection_syntax(t_token *current);
void					compact_args_array(char **args, int count);
int						process_redirections(t_gc *gc, t_cmd *cmd,
							t_token **current);
...
char					*process_unquoted_segment(t_expand_context *ctx,
							char *str, int *i);
char					*process_double_quote_segment(t_expand_context *ctx,
							char *str, int *i);
char					*process_single_quote_segment(t_gc *gc, char *str,
							int *i);
char					*expand_env_var(t_gc *gc, t_env *env, char *var_name);
char					*replace_exit_status(t_gc *gc, char *str,
							int exit_status);
void					link_tokens_to_original(t_token **tokens,
							t_token *token_to_split);
void					create_tokens_from_words(t_gc *gc, t_token **tokens,
							char **words, t_token *prev_token);
t_token					*create_and_configure_token(t_gc *gc, char *word,
							int has_space);
void					link_token_to_list(t_token **tokens, t_token **current,
							t_token *prev_token, t_token *new_token);
t_token					*find_token_position(t_token **tokens,
							t_token *token_to_split, t_token **prev_token);
void					split_token_by_whitespace(t_gc *gc, t_token **tokens,
							t_token *token_to_split);
void					mark_heredoc_delimiters(t_token *tokens);
void					set_token_properties(t_token *token, char *value,
							t_token_type type);
void					expand_env_var_token(t_gc *gc, t_env *env,
							t_token *current);
void					expand_exit_status_token(t_gc *gc, t_token *current);
void					expand_token_variables(t_gc *gc, t_env *env,
							t_token *current);
void					process_token_expansion(t_gc *gc, t_env *env,
							t_token *tokens);
...
void					cleanup_line_resources(char **line,
...
int						validate_input_and_tokenize(t_gc *gc, char *line,
							t_token **tokens);
int						execute_and_cleanup(t_gc *gc, t_env *env,
							t_pipeline *pipeline, int *exit_status);
int						handle_incomplete_pipe(t_gc *gc, t_env *env,
...
int						handle_multiple_heredocs(t_gc *gc, t_env *env,
							t_heredoc *heredoc_list);
void					setup_heredoc_child_process(t_gc *gc, t_env *env,
							int write_fd);
...
int						setup_redirections(t_exec_context *ctx, int *input_fd,
							int *output_fd);
int						handle_heredoc(t_gc *gc, t_env *env, char *delimiter,
							int is_quoted);
...
char					*build_full_path(t_gc *gc, char *path_dir,
							char *command);
char					*check_command_in_path(t_gc *gc, char *command,
							char *path_dir);
char					*handle_absolute_path(t_gc *gc, char *command);
char					*search_in_path_dirs(t_gc *gc, char **path_dirs,
							char *command);
...
void					setup_child_redirections(t_exec_context *ctx,
							int redirect_input_fd, int redirect_output_fd);
...
void					handle_command_not_found(t_gc *gc, t_env *env,
							char *cmd_name);
void					handle_file_descriptors(int input_fd, int output_fd);
int						wait_for_processes(pid_t *pids, int count);
int						handle_process_status(int status, int i, int count,
							int *sigint_received);
void					close_pipes(int *pipe_fds, int count);
